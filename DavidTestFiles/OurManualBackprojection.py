# -*- coding: utf-8 -*-
"""
Created on Mon Jul 30 01:47:17 2018
...
parsedData = pd.read_csv('DavidPositionModified.csv',low_memory=False, delimiter=',', header=0, encoding='ascii')
X = parsedData['X']
Y = parsedData['Y']
Z = parsedData['Z']
motion_data = []
...
#Parses Pulses Data
Pulses = []
with open('DavidPulseDataModified.csv', 'r') as csvfile:
...
RangeAxis = []
with open('DavidRangeAxisModified.csv','r') as csvfile:
    reader = csv.reader(csvfile,delimiter= ' ', quotechar = '|')
    for row in reader:

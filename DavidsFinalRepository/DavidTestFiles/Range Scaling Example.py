# -*- coding: utf-8 -*-
"""
Created on Mon Jul 23 16:35:11 2018
...
#Generating RCS unscaled data
UnscaledPulses = Pulses #Initialize UnscaledPulses
onewayRangeAxis  = RangeAxis/2 # Calculate One-way Range Axis
(num_pulses, num_range_bins) = Pulses.shape #Assigning ranges using shape  of Pulses data

for p in range(0, num_pulses): #Iterate over pulses
    for r in range(0, num_range_bins): #Iterate over range binss
        UnscaledPulses[p,  r] = Pulses[p, r]*1/((onewayRangeAxis[0, r])**4+1)
    ...
    for x in arange(LeftInterval,RightInterval,StepSize): #Iterates over x-coordinates
        ...
        for i in range(len(PlatformX)): #Iterates over platform positions
           ...
           # intensityx += Weight1*Pulses[i,  int(bin(actualRange(PlatformX[i], PixelCoord)))] + Weight2*Pulses[i,  int(bin(actualRange(PlatformX[i], PixelCoord)))+1] 
            intensityx += Weight1*UnscaledPulses[i,  int(bin(actualRange(PlatformX[i], PixelCoord)))] + Weight2*UnscaledPulses[i,  int(bin(actualRange(PlatformX[i], PixelCoord)))+1] 
